...
function backup_mydumper() {
    ...
    mkdir -p ${this_base_p}
    for dbname in $(mysql -u${db_user} -p${db_pass} -S ${socket} -e "show databases;"  | egrep -w -v "(Database|information_schema|performance_schema|mysql|test|log|common_schema)")
    do               
