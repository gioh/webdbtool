...
# @File_name: remote_db_execute2.py
# @File_path: E:\开源程序\my_dba_release\app\tools\remote_db_execute2.py
# @Author: 丁以然
# @Email:  dingyiran@szlanyou.com
# @Create_time:   2014-06-17 15:04:21
# @Last Modified by:   丁以然
# @Last Modified time: 2015-06-04 11:22:43
...
def log_w(text):
    logfile = "/tmp/my_dba_release.log"
...
def remote_exe_sql(
    ...
    remotefile = "/tmp/my_dba_exe_sql.sql"
    ...
    try:
        ...
        conm = r'''%s/mysql -u%s -p'%s' %s %s < %s''' % (
            config.mysql_client_path,v_db_user,v_db_pwd,v_db_socket, v_db_name, remotefile)
        ...
        print e.message
        text = "%s:%d Call Function:remote_exe_sql Error ! Error Reason: %s Sql:%s" % (v_host,v_db_port,e.message,v_exe_sql)
        log_w(text)
        print "\033[1;31;40m%s\033[0m" % text
